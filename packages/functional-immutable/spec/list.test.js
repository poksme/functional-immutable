...
describe('List', () => {
  ...
  it('of provides initial values', () => {
    var v = list.of('a', 'b', 'c');
    expect(list.get(0)(v)).toBe('a');
    expect(list.get(1)(v)).toBe('b');
    expect(list.get(2)(v)).toBe('c');
  });

  it('toArray provides a JS array', () => {
    var v = list.of('a', 'b', 'c');
    expect(list.toArray()(v)).toEqual(['a', 'b', 'c']);
  });

  it('can set and get a value', () => {
    var v = List();
    expect(list.get(0)(v)).toBe(undefined);
    v = list.set(0, 'value')(v);
    expect(list.get(0)(v)).toBe('value');
  });

  it('returns undefined when getting a null value', () => {
    var v = List([1, 2, 3]);
    expect(list.get(null)(v)).toBe(undefined);

    var o = List([{ a: 1 },{ b: 2 }, { c: 3 }]);
    expect(list.get(null)(o)).toBe(undefined);
  });

  it('counts from the end of the list on negative index', () => {
    var i = list.of(1, 2, 3, 4, 5, 6, 7);
    expect(list.get(-1)(i)).toBe(7);
    expect(list.get(-5)(i)).toBe(3);
    expect(list.get(-9)(i)).toBe(undefined);
    expect(list.get(-999, 1000)(i)).toBe(1000);
  });

  it('coerces numeric-string keys', () => {
    // Of course, TypeScript protects us from this, so cast to "any" to test.
    var i = list.of(1, 2, 3, 4, 5, 6);
    expect(list.get('1')(i)).toBe(2);
    expect(fp.flow(list.set('3', 10), list.get('3'))(i)).toBe(10);
    // Like array, string negative numbers do not qualify
    expect(list.get('-1')(i)).toBe(undefined);
    // Like array, string floating point numbers do not qualify
    expect(list.get('1.0')(i)).toBe(undefined);
  });
